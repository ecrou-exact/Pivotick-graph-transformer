...
`demo/` is the local Vite site this strategy allows (`cd demo && npm run dev`), three pages: `index.html` (Home — the landing page, kept at this filename deliberately so it's what GitHub Pages' project root serves), `demo.html` (Demo — a floating fixture picker over a full-UI Pivotick instance, switching between `demo/fixtures/misp/**`), and `docs.html` (Docs — one section per MISP concept, each a small hand-written fixture under `demo/fixtures-docs/` paired with a live "basic" Pivotick preview). All three share a top nav (`demo/src/siteHeader.ts`) and a dark/light toggle persisted across pages in `localStorage` (`demo/src/theme.ts`) — see README's "Demo & Docs" section. Deployed to GitHub Pages on every push to `main` touching `demo/**`/`packages/**` (`.github/workflows/deploy-demo.yml`), which builds with `GITHUB_PAGES_BASE=/<repo-name>/` so `demo/vite.config.ts`'s `base` resolves correctly under a project-page subpath.
