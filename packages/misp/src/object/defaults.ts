...
// as event/defaults.ts (shape: 'none', the real look drawn by
// buildIconLabelCard via `html`). Unlike Event, the icon itself isn't
// fixed: import.ts picks `objects/<name>.svg` when misp-iconify has one for
// that Object's `name` (e.g. "domain-ip", "file", ...), falling back to
// this generic `object` icon otherwise.
